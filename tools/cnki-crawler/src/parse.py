...
def _looks_like_list_or_detail_content(html: str) -> bool:
    """正常列表/详情正文（论文标题常含「安全验证」等词，不能当验证页）。"""
    return (
        'id="briefBox"' in html
        or "result-table-list" in html
        or "pagerTitleCell" in html
        or 'id="hidTurnPage"' in html
        or 'class="no-content"' in html
        or "暂无数据" in html
        or "icon-collect" in html
        or 'class="wx-tit"' in html
        or 'id="doc-title"' in html
        or 'class="doc-top"' in html
    )


def detect_captcha_or_login(html: str) -> None:
    # 先认内容页：列表 HTML 里论文题名「…与安全验证」会误伤
    if _looks_like_list_or_detail_content(html):
        return
    lower = html.lower()
    if (
        "验证码" in html
