...
class Checkpoint:
    ...
    def prepare_daily_detail_quota(self, limit: int) -> bool:
        """重置日计数（如跨日）；若已达上限返回 False。"""
        from datetime import date

        with self._lock:
            today = date.today().isoformat()
            if self.details_date != today:
                self.details_date = today
                self.details_today = 0
                self._save_unlocked()
            return self.details_today < limit

    def bump_daily_detail(self) -> None:
        from datetime import date
